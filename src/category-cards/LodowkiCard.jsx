...
export default function LodowkiCard({ item, editItem, deleteItem, role }) {
    const [openPhoto, setOpenPhoto] = useState(false);

    return (
        <>
            <div className="card card-premium card-hover-effect fade-in-up fridge-card">
                <div className="flex justify-between items-start mb-3">
                    <h2 className="text-lg font-semibold text-slate-800 truncate max-w-[70%]">{item.name}</h2>
                    <div className="flex gap-1">
                        <span className="badge-modern bg-green-900/50 border-green-500/50 text-green-400">
                            {item.category}
                        </span>
                        {item.quantity && (
                ...
                <div className="space-y-2 text-sm">
                    ...
                    {item.linknadysk && (
                        <div className="pt-2">
                            <button
                                onClick={() => window.open(item.linknadysk, '_blank')}
