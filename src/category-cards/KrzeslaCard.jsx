...
export default function KrzeslaCard({ item, editItem, deleteItem, role }) {
    const [openPhoto, setOpenPhoto] = useState(false);

    return (
        <>
            <div className="card card-premium card-hover-effect fade-in-up chair-card">
                <div className="flex justify-between items-start mb-3">
                    <h2 className="text-lg font-semibold text-slate-800 truncate max-w-[70%]">{item.name}</h2>
                    <div className="flex gap-1">
                        <span className="badge-modern bg-gray-700/50 border-gray-500/50 text-gray-300">
                            {item.category}
                        </span>
                        {item.quantity && (
